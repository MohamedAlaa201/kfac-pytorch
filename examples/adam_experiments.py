...
class Experiment:
    def __init__(self, model, convergence_criteria_enabled, kfac_enabled):
        self.model = model
        self.kfac_enabled = kfac_enabled
        self.convergence_criteria_enabled = convergence_criteria_enabled
        self.run()
    ...
    def run(self) -> None:
        ...
        if args.verbose:
            summary(self.model, (args.batch_size, 3, 32, 32), device=device)
 ...
 """
        ...
        for epoch in range(args.resume_from_epoch + 1, args.epochs + 1):
            ...
            val_loss = engine.test(
                epoch, self.model, loss_func, val_loader, args
            ).avg
            ...
            if self.convergence_criteria_enabled:
...
def test_model(model):
    adams_cifar10 = Experiment(
        model, kfac_enabled=True, convergence_criteria_enabled=False
    )
    adams_cifar10_convergence = Experiment(
        model, kfac_enabled=True, convergence_criteria_enabled=True
    )
    # KFAC till convergence
    kfac_cifar10_convergence = Experiment(
        model, kfac_enabled=True, convergence_criteria_enabled=True
    )

    # KFAC for 100 epochs
    kfac_cifar10 = Experiment(
        model, kfac_enabled=True, convergence_criteria_enabled=False
    )
